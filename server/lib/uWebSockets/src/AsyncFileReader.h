...
public:
    ...
    std::string_view peek(unsigned long offset) {
        ...
        if (hasCache && offset >= cacheOffset && ((unsigned long)(offset - cacheOffset) < cache.length())) {
            /* Cache hit */
            ...
            unsigned long chunkSize = std::min<unsigned long>((unsigned long)(fileSize - offset), (unsigned long)(cache.length() - offset + cacheOffset));
    ...
    void request(unsigned long offset, std::function<void(std::string_view)> cb) {
        ...
        //std::async(std::launch::async, [this, cb, offset]() {
            ...
            fin.seekg((long long) offset, fin.beg);
            fin.read(cache.data(), (long) cache.length());
            ...
            loop->defer([this, cb, offset]() {
                ...
                unsigned long chunkSize = std::min<unsigned long>(cache.length(), (unsigned long)(fileSize - offset));
